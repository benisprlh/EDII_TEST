    ...
    static async create(req, res, next) {
      ...
      static async update(req, res, next) {
        ...
        try {
          ...
          const newEducation = await Education.update({level, name, major, graduateDate, ipk});
      ...
      static async delete(req, res, next) {
        try {
          ...
          res.status(200).json({ message: `${education.title} success to delete` });
        } catch (error) {
          next(error);
        }
      }
      ...
      static async getEducation(req, res, next) {
        try {
            let dataEducation = Education.findByPk(req.params.id);
